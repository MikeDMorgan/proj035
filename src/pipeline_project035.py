...
effects on the germline transcriptome?
...
Aligned short read sequencing data, compressed, i.e. *.bam
...
The pipeline outputs tables of differentially expressed annotations, MA-plots, sample clustering by
Pearson correlation, and performs a PCA, visualising the first 2 axes of variation.

It also generates an in silico spike-in experiment by iteratively shuffling the input table of gene counts
to create a simulated dataset.  Shuffling is performed iteratively to create a range of log-fold differences
between the experimental conditions.  It therefore conserves the within-condition variance structure of the data. 
